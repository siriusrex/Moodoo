...
function init() {
    ...
    if (environment.screenWidth < 700) {
        ...
        $(document).width(environment.screenWidth * 0.8);
        $(document).height(environment.screenHeight * 0.8);
    ...
    //now we know screen size - if mobileSize set constant background size
	if (environment.mobileSize){
		$('#bgtest').width(environment.screenWidth*1.2);
		$('#bgtest').height(environment.screenHeight*1.4);
		$('#bgtest').css('background-size', environment.screenWidth*1.2+'px '+environment.screenHeight*1.2+'px');
    ...
    if (environment.touchInput) {
	    inputs.hammertime = new Hammer(mooderatorGraph);
	    inputs.hammertime.get('pinch').set({ enable: true });
	    var mooderatorGraph=document.getElementById('mooderatorGraph');
	    inputs.hammerController=inputs.hammertime.Manager(mooderatorGraph);
	    var pinch=inputs.hammertime.pinch();
	    inputs.hammerController.add([pinch]);
	    inputs.hammerController.on('pinchmove', function(event){
