...
function janrainWidgetOnload() {
   ...
   janrain.events.onProviderLoginStart.addHandler(function(response) {
		...
		console.log('starting Janrain window timer');
                    
		 //start timer to catch janrain stall
		 janrainWindowTimerCheck();
	});
...
function setMiniGraphSlider(){
	...
	//$('#miniGraphSlider').offset({top: $('#miniGraphSlider').offset().top, left: 0});
...
function onViewportResize(){
	...
	$('#miniGraphSlider').position({top: 0, left: newSliderPos});
...
function onMouseUpSlider(e){
	...
	sliderPosRatio=$('#miniGraphSlider').position().left/($(window).width()-110);
	mooderatorZoomRatio=mooderatorWidth/($(window).width()-110);
	
	console.log('at mouseup, sliderWidthRatio='+sliderWidthRatio+', miniGraphSlider.position().left='+$('#miniGraphSlider').position().left+', sliderPosRatio='+sliderPosRatio);
