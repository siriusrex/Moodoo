...
function deleteUpdateConfirm(updateNo){
	...
	$('#updateBox'+updateNo).html('<span class="confirmDeleteSpan">Are you sure you want to delete this update? <br/> <a href="javascript:deleteUpdate('+me.updates[updateNo].updateID+', '+updateNo+')" class="confirmDeleteButton">Delete</a><a href="javascript:cancelUpdateDelete('+updateNo+')" class="confirmDeleteButton">Cancel</a></span>');
	
}

function deleteUpdate(updateID, updateNo){
	logIt('deleting  updateID='+updateID);
	...
	$.ajax({
                ...
                error: function(data, status, xhr){
                	...
                	$('#communication').html('<h1>Oops! Something went wrong.</h1><p>We couldn\'t talk to the Moodoo server. <br/>Please check your connection and try again in a few minutes.<br/> Your mood has not been deleted.</p>');
                	showDiv('communication');
                	logIt ('data='+data+', status='+status+', xhr='+xhr);
                	for (var c in data){
	                	logIt(c+'='+data[c]);
...
function showRecentUpdates(noToShow){
	...
	if (noToShow==4){
		...
		$('#recentUpdates').animate({height: 560}, 500, function (){});
		
		$('#recentUpdateTable').html('');
		
		$('#recentUpdateTable').append('<div id="updateCol1" class="updatesColumn">');
		$('#updateCol1').height(350);
		for (var v=1; v<noToShow; v++){
			
			$('#updateCol1').append('<span id="updateBox'+parseInt(noOfUpdates-v).toString()+'" class="showUpdatesBox"><span style="text-align: left; width: 48px; height: 48px; display: block; float: left; margin:15px; background-image: url(images/'+theMoods[me.updates[noOfUpdates-v].mood]+'_roll.png); background-position-y:-47px"></span><span style="display:block;  font-weight:bold; margin-top:10px">'+theMoods[me.updates[noOfUpdates-v].mood]+'</span><span style="font-size:0.7em; margin-top: 5px; display:block">'+convertDate(me.updates[noOfUpdates-v].time)+'</span> <span class="recentUpdateBoxStatus">'+unescape(me.updates[noOfUpdates-v].status)+'</span><a class="deleteButton" href="javascript:deleteUpdateConfirm('+parseInt(noOfUpdates-v).toString()+')">x</a></span></span>');
			$('updateBox'+parseInt(noOfUpdates-v).toString()).data('boxNumber', parseInt(noOfUpdates-v));
			$('updateBox'+parseInt(noOfUpdates-v).toString()).data('updateID', me.updates[noOfUpdates-v].updateID);
		}
		...
		$('#recentUpdateTable').animate({
			...
			for (var v=4; v<noToShow; v++){
				
				$('#updateCol2').append('<span id="updateBox'+parseInt(noOfUpdates-v).toString()+'" class="showUpdatesBox"><span style="text-align: left; width: 48px; height: 48px; display: block; float: left; margin:15px; background-image: url(images/'+theMoods[me.updates[noOfUpdates-v].mood]+'_roll.png); background-position-y:-47px"></span><span style="display:block;  font-weight:bold; margin-top:10px">'+theMoods[me.updates[noOfUpdates-v].mood]+'</span><span style="font-size:0.7em; margin-top: 5px; display:block">'+convertDate(me.updates[noOfUpdates-v].time)+'</span> <span class="recentUpdateBoxStatus">'+unescape(me.updates[noOfUpdates-v].status)+'</span><a class="deleteButton" href="javascript:deleteUpdateConfirm('+parseInt(noOfUpdates-v).toString()+')">x</a></span></span>');
				
				
			}
			$('#updateCol2').height(350);
			$('#updateCol2').animate({opacity: 1}, 500, function (){});
		...
		$('#recentUpdates').animate({height: (noToShow/2)*110+200}, 500, function (){});
		$('#recentUpdateTable').animate({
			
			width: 760,
			height: (noToShow/2)*110
			...
			$('#updateCol1').height((noToShow/2)*110);
			
			$('#updateCol2').height((noToShow/2)*110);
			$('#updateCol2').animate({opacity: 1}, 500, function (){});
			$('#showMoreUpdatesButton').css('display', 'none');			
			$('#showRecentUpdatesButton').css('display', 'block');			
...
function drawGraph(moodData){	   
    ...
    for (var i=0; i<moodData.length; i++){
		...
		if (!(nextGraphX<0)){
			...
			canvasPoints.push({x:graphX, y:graphY, number:i, mood:moodData[i]["mood"], group:pointGroup});
...
function createMoodPie(highlight){
	...
	for (var p in moodFrequency){
		if (moodFrequency[p]>0){
			...
			pieSlicePercentages.push(new Object({number:0, colour:null}));
		}
	}
...
function showToolTip(posX, posY, number){
	...
	$('#mooderatorPopUpLabel').html("<span style='font-size: 0.7em; color: #fff; margin:0px; padding:0px'>"+formalTime+"</span> <br/> <span id='labelRoundBox' style='background-color:"+colourArray[me.updates[arrayIndex].mood]+"'>"+theMoods[me.updates[arrayIndex].mood]+"</span> "+unescape(me.updates[arrayIndex].status));
