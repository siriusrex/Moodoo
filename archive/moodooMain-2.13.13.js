...
function init() {
	...
	if ($('#bgtest').width()<environment.screenWidth){
		...
		$('#bgtest').css('background-size', '100% 100%');
...
function onTouchMoveGraph(e){
	...
	//gesture simulation
	/*if (multiTouchDistanceAtStart){
		logIt('onTouchMoveGraph: multiTouchDistanceAtStart='+multiTouchDistanceAtStart);
		var noOfTouches=e.originalEvent.targetTouches.length;
	
		if (noOfTouches>1){
			var touch1X=parseFloat(e.originalEvent.targetTouches[0].pageX-e.target.offsetLeft);
			var touch2X=parseFloat(e.originalEvent.targetTouches[noOfTouches-1].pageX-e.target.offsetLeft);
			multiTouchDistanceAtTouchMove=touch2X-touch1X;
			currentMultiTouchRatio=(multiTouchDistanceAtTouchMove/multiTouchDistanceAtStart);
			
			zoomChangeTouch(currentMultiTouchRatio, touch1X);
		}
	}*/
