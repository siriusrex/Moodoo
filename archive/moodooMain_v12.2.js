...
var baseURL='https://www.moodoo.net/dev/auth';
...
(function() {
    ...
    janrain.settings.tokenUrl = 'https://www.moodoo.net';
...
function drawMooderatorControl(){
	...
	$('#miniGraph').attr('width', $(window).width()-100);
	$('#miniGraph').attr('height', 80);
	$('#miniGraph').css('display', 'block');
	$('#miniGraph').css('width', $(window).width()-100+'px');
...
function drawMiniGraph(moodData){	   
    ...
    ctxMini.moveTo(baseX, (graphMiniInnerHeight-(moodData[0]["mood"]*heightRatio)));
    ctxMini.beginPath();
    console.log('moodData.length='+moodData.length);
    ...
    for (var i=0; i<moodData.length; i++){
		...
		var graphX=(graphMiniPoints[i]*mooderatorWidth+baseX+5);
		var nextGraphX=(graphMiniPoints[i+1]*mooderatorWidth+baseX+5);
	...
	ctxMini.stroke();
	$('#miniGraphSlider').css('width', $('#miniGraph').width()+10);	
	$('#miniGraphSlider').css('left', '0px');	
...
function setMiniGraphSlider(){
	...
	sliderWidth=$('#miniGraphSlider').width();
	sliderPos=$('#miniGraphSlider').offset().left;
}
...
function onViewportResize(){
	...
	console.log('sliderWidth='+sliderWidth+', sliderPos='+sliderPos);
	
	
	var newViewportWidth=$(window).width()-110;
	
	var newSliderWidth=(newViewportWidth/viewportWidth)*sliderWidth;
	
	var newSliderPos=(newViewportWidth/viewportWidth)*sliderPos;
	
	//var newMooderatorWidth=(newViewportWidth/viewportWidth)*mooderatorWidth;
	
	
	//$('#miniGraphSlider').width(newSliderWidth);
	
	//$('#miniGraphSlider').css('margin-left', newSliderPos+'px');
...
function onMouseDownSliderBody(e){
	...
	var xpos=e.pageX//;-$('#miniGraph').offset().left;
	$('#miniGraphSlider').css('cursor', ' -webkit-grabbing');
	dragOffset=xpos-parseInt($('#miniGraphSlider').css('margin-left'));
...
function onMouseMoveSlider(e, leftOrRight){
	...
	if (leftOrRight=='left'){
		...
		if (xpos>0){
			...
			$('#miniGraphSlider').css('margin-left', xpos+$('#miniGraph').offset().left+'px');
		...
		if (xpos<$('#miniGraph').width()){
			var originalMargin=parseInt($('#miniGraphSlider').css('margin-left'));
			var endOffset=$('#miniGraph').width()-xpos;
			var newWidth=$('#miniGraph').width()-originalMargin-endOffset;
			//console.log('minigraph width='+$('#miniGraph').width()+', originalMargin='+originalMargin+', endOffset='+endOffset+', newWidth='+newWidth);
			$('#miniGraphSlider').css('width', newWidth+50+'px');
		}
	}
		...
		var leftBarrier=$('#miniGraph').offset().left;
		var rightBarrier=$('#miniGraph').width()+$('#miniGraph').offset().left;
		if (newMargin>leftBarrier && ($('#miniGraphSlider').width()+newMargin<rightBarrier)){
			$('#miniGraphSlider').css('margin-left', newMargin+'px');
...
function showToolTip(posX, posY, number){
	...
	$('#mooderatorPopUpLabel').html("<span style='font-size: 0.7em; color: #fff; margin:0px; padding:0px'>"+me.updates[arrayIndex].time+"</span> <br/> <span id='labelRoundBox' style='background-color:"+colourArray[me.updates[arrayIndex].mood]+"'>"+theMoods[me.updates[arrayIndex].mood]+"</span> "+me.updates[arrayIndex].status);
