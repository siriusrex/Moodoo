...
//physical environment vars

var environment=new Object();

environment.screenWidth = 0;
environment.screenHeight = 0;
environment.mobileSize = false;
environment.touchInput = false;
environment.backgroundRatio=0.75;

//background image urls - constants
...
//var to hold instance of hammer.js for cross-broswer touch events
inputs.hammertime=null;
...
function init() {
    ...
    getEnvironment();
    logIt('environment.screenWidth=' + environment.screenWidth);

        
   
    /*
	if ($('#bgtest').width()<environment.screenWidth){
		$('#bgtest').width(environment.screenWidth);
		$('#bgtest').height(environment.screenWidth*environment.backgroundRatio);
		$('#bgtest').css('background-size', '120%');
		
	}
	if ($('#bgtest').height()<environment.screenHeight){
		$('#bgtest').height(environment.screenHeight*1.4);
		$('#bgtest').width(environment.screenWidth*1.4*(1/environment.backgroundRatio));
	}*/
    ...
    //if touchscreen, set up cross-platform pinch event on mooderatorGraph via hammer.js
    if (environment.touchInput) {
	   logIt('adding hammer pinch zoom function');
	    var mooderatorGraphHammer=document.getElementById('mooderatorGraph');
	   	inputs.hammertime=new Hammer(mooderatorGraphHammer);
	   	inputs.hammertime.get('pinch').set({ enable: true });
	    inputs.hammertime.on('pinchstart', function(event){
		    onGestureStartGraph(event);
	    });
	    inputs.hammertime.on('pinchmove', function(event){
		    
		 	logIt('got hammer scale on mooderator: scale='+event.scale); 
		 	logIt('got hammer event.center.pageX='+event.center.pageX); 
		 	zoomChangeTouch(event.scale, event.center.pageX); 
	  
	    });
	    inputs.hammertime.on('pinchend', function(event){
		    onGestureEndGraph(event);
	    });
    ...
    logIt('environment.touchInput='+environment.touchInput);
    
    //workaround for IE11 smooth scrolling bug
	...
	$('#communication').css('height', 'auto');
	$('#communication').data('myHeight', 'auto');
    ...
    $('#status').keypress(function() {
        ...
        if (environment.mobileSize){
	        newWidth= statusValue.length * 20;
        }
        if (newWidth < 100) {
            newWidth = 100;
        }
        if (newWidth > environment.screenWidth*0.7){
	        newWidth=environment.screenWidth*0.7;
        }
        $('#status').animate({
            width: newWidth
        }, 100, function() {
...
function getEnvironment(){
	environment.screenWidth = $(window).width();
    environment.screenHeight = $(window).height();
    
    if (environment.screenWidth < 700) {
        logIt('mobile or tablet sized screen detected');
        environment.mobileSize = true;
	...
	//check interface input type  - is it a touchscreen?
    
    environment.touchInput = !!('ontouchstart' in window);
}
////////////////////////////////////////////////////////////
...
function restyleJanrain() {
	...
	$('#janrainEngageEmbed').css('margin-left', 'auto');
	$('#janrainEngageEmbed').css('margin-right', 'auto');
    ...
    if (environment.mobileSize) {
	    $('#janrainEngageEmbed').css('width', '15em');
// 	    $('#janrainEngageEmbed').css('margin-left', 'auto');
// 	    $('#janrainEngageEmbed').css('margin-right', 'auto');
...
function submitContactForm(){
	//send details from contact form to mailer script
	logIt('submitContactForm running, sending:'+$('#name').val()+','+$('#email').val()+','+$('#subject').val()+','+$('#message').val());
	$.ajax({
            type: "POST",
            url: "moodoo_mail.php",
            data: {
                "contactFirstName": $('#name').val(),
                "returnEmail": $('#email').val(),
                "subject":  $('#subject').val(),
                "contactMessage": $('#message').val()
            },
            success: function(res, status, xhr) {
	           logIt('submitContactForm: contact form data sent.');
	          
                    logIt('res='+res.toString());
	...
	hideContactForm();
	createDisplay();
	$('#communication').html="<p>Your message has been sent.</p>";
	showDiv('communication');
}
...
function deleteUpdateConfirm(updateNo) {
    ...
    $('#updateBox' + updateNo).html('<span class="confirmDeleteSpan"><p>Are you sure you want to delete this update?</p> <a href="javascript:deleteUpdate(' + parseInt(me.updates[updateNo].updateID) + ', ' + updateNo + ')" class="confirmDeleteButton">Delete</a><a href="javascript:cancelUpdateDelete(' + updateNo + ')" class="confirmDeleteButton">Cancel</a></span>');
	$('#updateBox' + updateNo).css('text-align', 'center');
}
...
function cancelUpdateDelete(updateNo) {
    ...
    $('#updateBox' + updateNo).toggleClass("transparent");
	$('#updateBox' + updateNo).css('text-align', 'left');
}
...
function showRecentUpdates(noToShow) {
    ...
    if (noToShow == 3) {
        ...
        for (var v = 0; v < noToShow; v++) {
            ...
            var updateBoxText='<span id="updateBox' + loopNo.toString() + '" class="showUpdatesBox blur">'+makeUpdateBox(loopNo)+'</span>';   
        ...
        if (!environment.mobileSize){
	        ...
	        $('#recentUpdates').css('height', 'auto');
	        $('#recentUpdateTable').css('height', 'auto');
			$('#updateCol1').css('height', 'auto');
        }
	...
	$('#recentUpdates').css('display', 'block');
	$('#recentUpdates').data('hidden', false);
	$('#recentUpdates').css('visibility', 'visible');
	$('#recentUpdates').css('opacity', 1);
...
function drawMooderator() {
	...
	//logIt('drawMooderator: environment.mobileSize='+environment.mobileSize);
    ...
    if (canvas) {
        logIt('drawMooderator: canvas exists');
        if (canvas.getContext) {
            logIt('drawMooderator: canvas.getContext exists');
            //new canvas context
...
function setMiniGraphSlider() {
    ...
    $('#wrapper').on('touchstart', function(e) {
        logIt('touchstart on wrapper');
        onMouseUpDocument(e);
    });
    
    

	$('#miniGraphSlider').width($('#miniGraph').width());
...
function zoomChangeTouch(gestureScale, gestureX){
    logIt('zoomChangeTouch: gestureScale='+gestureScale+', gestureX='+gestureX)
    logIt('zoomChangeTouch: gestureScale*mooderatorWidth='+gestureScale*mooderatorWidth);
...
function getPieWidth(){
	var pieWidth;
	getEnvironment();
	//test portrait or landscape
	if (environment.screenHeight>environment.screenWidth){
        pieWidth = parseFloat(viewportWidth * 1.1);
    } else {
       pieWidth = parseFloat(viewportWidth * 0.6);
    }
...
function onMouseMoveGraph(e) {
    ...
    testHitGraphDot(mousePos);
 }
 
 function testHitGraphDot(mousePos){
    var hit = false;
...
var touchStartX=0;
var touchMoveX=0;
var multiTouchDistanceAtStart=0;
var multiTouchDistanceAtTouchMove=0;
var currentMultiTouchRatio=0;
...
function onTouchStartGraph(e){
	...
	logIt('onTouchStartGraph: noOfTouches='+noOfTouches);
	baseXAtTouchStart=baseX;
	
	logIt('onTouchStartGraph: baseXAtTouchStart='+baseXAtTouchStart);
	...
	if (noOfTouches>1){
		...
		touchMidPointAtStartGesture=touch1X;
		baseXAtStartGesture=baseX;
		
		
		//get distance between touches so that we can work out percentage change in distance during pinch (for android which lacks touch gesture)
		
		multiTouchDistanceAtStart=touch2X-touch1X;
		
		logIt('onTouchStartGraph: multiTouchDistanceAtStart='+multiTouchDistanceAtStart);	
		...
		touchStartY=parseFloat(e.originalEvent.targetTouches[0].pageY-e.target.offsetTop);
		logIt('touchStartX='+touchStartX);
		multiTouchDistanceAtStart=null;
		var touchPos = {
		  x: touchStartX,
		  y: touchStartY
    	};
		...
		testHitGraphDot(touchPos);
	}
	
	
}
function onTouchMoveGraph(e){
	logIt('onTouchMoveGraph running');
	//single touch drag scroll
	touchMoveX=parseFloat(e.originalEvent.targetTouches[0].pageX-e.target.offsetLeft);
	logIt('onTouchMoveGraph: touchMoveX='+touchMoveX);
	scrollAmount=(touchStartX-touchMoveX);
	logIt('onTouchMoveGraph: scrollAmount='+scrollAmount);
	scrollGraph(scrollAmount);
	
	
	
	logIt('onTouchMoveGraph: e.originalEvent.targetTouches.length='+e.originalEvent.targetTouches.length);
...
function onGestureStartGraph(e){
    ...
    logIt('onGestureStartGraph: touch gesture start');
    mooderatorWidthAtStartGesture=mooderatorWidth;
...
function onGestureEndGraph(e){
    e.preventDefault();
    logIt('onGestureEndGraph: touch gesture end');
    $('#mooderatorGraph').on('touchmove', onTouchMoveGraph);
}

function onGestureChangeGraph(e){
   e.preventDefault();
    logIt('onGestureChangeGraph: touch gesture change');
...
function showToolTip(posX, posY, updateNumber, canvasNumber) {
	...
	if (!environment.mobileSize){
		...
		$('#mooderatorPopUpLabel').css('text-align', 'left');
		$('#mooderatorPopUpLabel').css('margin-left', 'auto');
		$('#mooderatorPopUpLabel').css('margin-right', 'auto');
...
function hideToolTip() {
	...
	if(environment.mobileSize){
		 $('#mooderatorPopUpLabel').css('display', 'none');
	}
...
function hideAllSections(){
	...
	$('.section').each(function(index) {
		logIt('hideAllSections: hiding section '+$(this).attr('id')+'via jquery class loop');
		if ($(this).data('hidden') == false) {
	        $(this).data('hidden', true);
	        logIt('hiding section ' + $(this).attr('id'));
	
	
	        //store height to return to on reshow
	        if ($(this).outerHeight() > 0) {
	            $(this).data('myHeight', $(this).height());
...
function hideDiv(divname, swapper) {
    ...
    if ($('#' + divname).data('hidden') == false) {
        ...
        //store height to return to on reshow
        if ($('#' + divname).outerHeight() > 2) {
            
            	$('#' + divname).data('myHeight', $('#' + divname).outerHeight());
		}
